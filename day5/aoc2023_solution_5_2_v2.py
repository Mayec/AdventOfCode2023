# Trying to optimize by prunning maps in reverse order. But can't wrap my head around it, to check what it should do in practice.
...
def main():
	...
	#Create pruned maps (pmaps)
	# Identify lowest destination range in last map:
	lastMap = maps[-1]
	lowest = -1
	i = 0
	for r in lastMap.ranges:
		if  lowest == -1:
			lowest = r.startDestination
			lowestIdx = i
		elif r.startDestination < lowest:
			lowest = r.startDestination
			lowestIdx = i
		i += 1
	
	# Prune from last map any ranges that are not the lowest. Store in pmaps list (reversed)
	lowestFinalRange = maps[-1].ranges[lowestIdx]
	pmaps = []
	pmap = Map()
	pmap.ranges.append(lowestFinalRange)
	pmaps.append(pmap)
	
	# Add wildcard non-conversion range:
	wildStart = 0
	wildLength = lowestFinalRange.startDestination
	if wildLength > 0:
		pmap.ranges.append( Range(wildStart, wildStart, wildLength) )
	
	#Translate lowest range throughout other maps (in reverse order)
	prevPmap = pmaps[0]
	
	for i in range(0, len(maps)-1):
	#for i in range(0,1): # TEST single
		mapnum = len(maps) - i -2
		pmap = Map()
		pmaps.append(pmap)
		
		print("---")
		print('Prunning map ' + str(mapnum))
		
		lowest = -1
		for rdst in prevPmap.ranges:	
			rdstmin = rdst.startDestination
			rdstmax = rdst.startDestination + rdst.length
			
			for rsrc in maps[mapnum].ranges:
				print("here")
				rsrcmin = rsrc.startDestination
				rsrcmax = rsrc.startDestination + rdst.length
				
				overlapStart = max(rdstmin, rsrcmin)
				overlapEnd = min(rdstmax, rsrcmax)
				overlapLength = (overlapEnd - overlapStart)
				
				#print("   overlapStart: " + str(overlapStart))
				#print("   overlapEnd: " + str(overlapEnd))
				#print("   overlapLength: " + str(overlapLength))
				print("   Overlap (start, end, length): " + str(overlapStart) + ", " + str(overlapEnd) + ", " + str(overlapLength))	
					
				
				if overlapLength > 0:
					conversion = rsrc.startSource - rsrc.startDestination
					newStartDestination = overlapStart
					newStartSource = overlapStart + conversion
					newLength = overlapLength
					pmap.ranges.append( Range(newStartDestination, newStartSource, newLength) )
					
					print("   New Range: " + str(newStartDestination) + ", " + str(newStartSource) + ", " + str(newLength))	
					
					if  lowest == -1:
						lowest = newStartSource
					elif newStartSource < lowest:
						lowest = newStartSource
	...
	# LOG resulting pruned maps:
	print("---")
	i = 0
	for pmap in pmaps:
		i += 1
		print("Pruned map " + str(i))
		for r in pmap.ranges:
			print("Pruned range: " + str(r.startDestination) + ", " + str(r.startSource) + ", " + str(r.length))
